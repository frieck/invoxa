...
export async function getSupplier(): Promise<Supplier> {
  ...
  if (rows.length === 0) {
    ...
    return {
      ...
      payment_provider: '', bank_name: '', bank_address: '', bank_account: '',
      bank_routing: '', iban: '', swift: '', website: '', notes: '',
    };
  }
  return rows[0];
}
...
export async function saveSupplier(s: Omit<Supplier, 'id'>): Promise<void> {
  ...
  await db.execute(
    `UPDATE supplier SET
      ...
      email=?, phone=?, tax_id=?, payment_provider=?, bank_name=?, bank_address=?,
      bank_account=?, bank_routing=?, iban=?, swift=?, website=?, notes=?
    WHERE id=1`,
    [
      s.name, s.address, s.address2, s.city, s.state, s.zip, s.country,
      s.email, s.phone, s.tax_id, s.payment_provider, s.bank_name, s.bank_address,
      s.bank_account, s.bank_routing, s.iban, s.swift, s.website, s.notes,
    ]
  );
}
